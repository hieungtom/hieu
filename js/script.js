...
const responsiveDropMenu = document.querySelector(".responsive-dropdown-menu");
const hideMenu = function () {
  responsiveDropMenu.classList.toggle("hidden");
};
hideMenu();
...
document.addEventListener("scroll", function () {
  ...
  navLinks.forEach((link) => {
    // forEach link element cycle
    link.classList.remove("active"); // Remove active class from all links
    if (link.getAttribute("href").includes(currentSection)) {
      // If link points to the currentSection ID
...
toggleMenuButton.addEventListener("click", hideMenu);

document.querySelectorAll(".navbar ul li a").forEach((link) => {
  link.addEventListener("click", hideMenu);
});

if (responsiveDropMenu.classList.contains("hidden")) {
  document.querySelector("main").addEventListener("click", function () {
    responsiveDropMenu.classList.add("hidden");
  });
}
